...
router.post("/create-first-admin", async (req, res) => {
  ...
  try {
    ...
    if (existing) {
      return res
        .status(403)
        .json({ error: "Un admin existe déjà. Utilise /login" });
    }
...
router.post("/verify", async (req, res) => {
  ...
  try {
    ...
    if (!token)
      return res.status(400).json({ error: "Code invalid or expire" });
...
router.put("/:id", authMiddleware, async (req, res) => {
  ...
  try {
    if (!email && !password) {
      return res
        .status(400)
        .json({ error: "One field (email or password) must be provided" });
    }
    const admin = await updateAdmin(Number(id), email, password);
    res.json(admin);
  } catch (err) {
    res
      .status(err.message.includes("Email already used") ? 409 : 400)
